...
$conn->query("set names 'utf8'");
 ...
 $queryGetDetails = $conn->query("SELECT * FROM Preferences WHERE ID=1");
   ...
   if ($queryGetDetails->num_rows >= 0) {
     ...
     $queryGetHeaders = $conn->query("SELECT * FROM Page WHERE Name='Preferences1'");
     ...
     $queryGetDetails2 = $conn->query("SELECT * FROM Preferences WHERE ID=2");
       ...
       if ($queryGetDetails2->num_rows >= 0) {
         while($row = $queryGetDetails2->fetch_assoc()) {
           ...
           $word2=$row["Value2"];
           $word3=$row["Value3"];
           array_push($array,$word1,$word2,$word3);
          }
         }
