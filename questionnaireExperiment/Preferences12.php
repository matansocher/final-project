...
<div class="container1">
      ...
      <article class="container">
        <div class="formBox">
	      <form id="myForm" name="myForm" class="hebrew" action="pageAfterPreferences1.php" method="post" onsubmit="return validateForm()">
...
    success: function(data){
  ...
  $("#header").append(arr[11]);
  $("#description").append(arr[12]);
  $("#errorMessage").append(arr[13]);

  for (i = 6; i < 11; i++) {
    if (arr[i] !== "") {
      var actionName = getActionName(i+1);
      arrayOfNumbersOfValidFeelings.push(i+1-6);
      var add = "<h3><strong>" + arr[i] + ":</strong><label class='invalid' id='invalid" + (i+1) + "'></label></h3>";
      add += "<input type='radio' name='" + (actionName) + "'value='1'> " + arr[15] + "<br>";
      add += "<input type='radio' name='" + (actionName) + "'value='2'> " + arr[16] + "<br><hr>";
...
if(
  (selectG==false && arrayOfNumbersOfValidFeelings.includes(1))||(selectH==false && arrayOfNumbersOfValidFeelings.includes(2))||
